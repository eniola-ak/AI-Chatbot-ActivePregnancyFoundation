...
const NancyAvatar: React.FC<{ size?: 'sm' | 'md' }> = ({ size = 'sm' }) => {
  ...
  return (
    <div className={`${dim} bg-purple-50 rounded-full flex items-center justify-center flex-shrink-0 border border-purple-100`}>
      <img src="/apf-logo.png" alt="Nancy" className={`${imgSize} w-auto object-contain`} />
    </div>
  );
};

// ── YES / NO / EXPLAIN BUTTONS ─────────────────────────────────────────────
...
const QuickReplies: React.FC<{ options: QuickReply[]; onSelect: (v: string) => void }> = ({ options, onSelect }) => (
...
export const ChatInterface: React.FC = () => {
  ...
  const [userProfile, setUserProfile] = useState<UserProfile>({
    name: '', category: null, otherDetail: '',
    screeningComplete: false, screeningFlaggedYes: false,
  });
  ...
  // ── FINISH SCREENING ─────────────────────────────────────────────────────
  const finishScreening = useCallback((flagged: boolean, name: string, category: UserCategory) => {
    setUserProfile(prev => ({ ...prev, screeningComplete: true }));
    setOnboardingStep('chat');
    const flagNote = flagged ? 'Just remember to check in with your GP or midwife about the things we flagged. ' : '';
    const categoryLine = category === 'pregnant'
    ...
    setTimeout(() => {
      addMessage(nancyMsg(`That's all the health questions done — well done for getting through them, ${name}! 🎉\n\n${flagNote}It sounds like ${categoryLine}. What would you like to know?`));
    }, 400);
  }, [addMessage]);
  ...
  const handleScreeningAnswer = useCallback(async (
    ...
    // ── NOT SURE → use explainTerm from question, explain via LLM ────────
    if (isNotSure) {
    ...
    // ── YES + refer_immediately → stop screening ──────────────────────────
  ...
  const handleCategorySelect = useCallback((value: string, name: string) => {
    ...
    if (category === 'preconception' || category === 'professional' || category === 'supporter') {
    ...
    const questions = category === 'pregnant' ? PREGNANCY_SCREENING_QUESTIONS : POSTNATAL_SCREENING_QUESTIONS;
    setScreeningQuestions(questions);
    setScreeningIndex(0);
    setOnboardingStep('screening');
    setTimeout(() => {
      const firstQ = questions[0];
      // First question uses explainTerm from screeningQuestions directly
      addMessage(nancyMsg(
        firstQ.text,
        { showYesNo: true, explainLabel: firstQ.explainTerm }
      ));
    }, 400);
  }, [addMessage]);
  ...
  const handleSend = async () => {
    ...
    if (onboardingStep === 'ask_name') {
      const raw = trimmed.replace(/^(i'?m|my name is|call me|i am)\s+/i, '').trim();
    ...
    if (onboardingStep === 'ask_other_detail') {
      ...
      setOnboardingStep('chat');
      setTimeout(() => {
        addMessage(nancyMsg(`Thank you for sharing that, ${userProfile.name}! I'll do my best to help based on APF resources. What's your question?`));
      }, 400);
      return;
    }
    ...
    if (onboardingStep === 'chat') {
      addMessage(userMsg(trimmed));
      setIsLoading(true);
      const newHistory: ChatMessage[] = [...geminiHistory, { role: 'user', text: trimmed }];
      setGeminiHistory(newHistory);
      const response = await getGeminiResponse(newHistory, userProfile.name, userProfile.category);
      if (isUnknownResponse(response)) {
        logUnknownQuestion(trimmed, userProfile.category ?? 'unknown', userProfile.name);
      }
  ...
  const showTextInput = onboardingStep !== 'ask_category' && onboardingStep !== 'screening';
    ...
    <div className="flex flex-col h-[75vh] bg-white rounded-3xl shadow-xl border border-purple-100 overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-5 py-4 bg-white border-b border-purple-50 flex-shrink-0">
        ...
        <div>
          ...
          <p className="text-[10px] text-purple-500 font-semibold uppercase tracking-widest">Active Pregnancy Foundation</p>
        </div>
              ...
              <div className="max-w-[85%]">
                {msg.role === 'nancy' && (
                  <div className="flex items-center gap-2 mb-1.5">
                    <NancyAvatar size="sm" />
                    <span className="text-[10px] font-bold text-purple-400 uppercase tracking-wider">Nancy</span>
                  </div>
                )}
                ...
                {/* Category quick replies */}
                {msg.role === 'nancy' && msg.showQuickReplies && isLatest && !categoryChosen && (
                  <QuickReplies options={msg.showQuickReplies} onSelect={val => handleCategorySelect(val, userProfile.name)} />
                )}

                {/* Yes/No/Explain buttons */}
                {msg.role === 'nancy' && msg.showYesNo && isLatest && onboardingStep === 'screening' && (
                  <YesNoButtons
                    explainLabel={msg.explainLabel}
                    onSelect={val => handleScreeningAnswer(val, screeningIndex, userProfile, screeningQuestions)}
                  />
                )}
              </div>
            </div>
          ...
          <div className="flex justify-start">
            <div className="flex items-center gap-2">
              ...
              <div className="bg-white border border-purple-50 px-4 py-3 rounded-2xl rounded-tl-none shadow-sm flex items-center gap-2 ml-2">
                ...
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">Nancy is thinking</span>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="p-4 bg-white border-t border-purple-50 flex-shrink-0">
          <div className="flex gap-2">
            <input type="text" value={input} onChange={e => setInput(e.target.value)}
              ...
              placeholder={onboardingStep === 'ask_name' ? 'Type your name...' : onboardingStep === 'ask_other_detail' ? 'Describe your situation...' : 'Ask Nancy anything...'}
              disabled={isLoading}
      ...
      {!showTextInput && (
        <div className="p-4 bg-white border-t border-purple-50 flex-shrink-0">
          <p className="text-[11px] text-center text-slate-400 font-medium">
            {onboardingStep === 'ask_category' ? '👆 Tap an option above to continue' : '👆 Use the buttons above to answer'}
          </p>
        </div>
      )}
    </div>
